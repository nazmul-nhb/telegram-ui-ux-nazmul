...
const Chats = () => {
	...
	const chats = chatData?.sort(
		(a, b) =>
			new Date(b?.creator?.updated_at) - new Date(a?.creator?.updated_at)
	);
	...
	return (
		<section className="flex flex-col bg-sideBG gap-2 p-2 h-[calc(100vh-56px)] pb-16 overflow-y-auto scrollbar-custom">
			{chats?.map((chat) => {
				...
				const chat_with = chat?.creator?.name || "Anonymous";
				return (
					<NavLink
						key={chat?._id}
						className={({ isActive }) =>
							isActive
								? "bg-chatMenuBG p-2 rounded-lg text-white"
								: "p-2 rounded-lg hover:bg-[#e5e5e671] transition-all duration-500"
						}
						to={`/chat/${chat?._id}?chat_with=${encodeURIComponent(
							chat_with
						)}`}
					>
						<div className="flex gap-2 items-center select-none">
							...
							<div className="w-full">
								...
								<div className="flex gap-2 justify-between">
									...
									<span>
                                        {moment(chat?.creator?.updated_at)
                                            .isSame(moment(), "day")
                                            ? moment(chat?.creator?.updated_at)
                                                .format("hh:mm A")
                                            : moment(chat?.creator?.updated_at)
